...
Status: needs-verification（原 `$EDITOR` 设计已被 ADR-0023 作废；只读页、片段开关与 `O` 清空均已落地；待真机跑一次"写覆写→重启内核→规则/日志无异常"）
Type: task
Blocked by: 01
...
1. ~~`O` 清空覆写（二次确认）~~ 已落地（2026-09-24，两次确认，档案名取 `active_profile` 以便坏文件也能删）。当时缺它不是审美问题：覆写文件名是档案名的 sha256，用户无法自己定位到

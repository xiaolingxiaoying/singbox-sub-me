# 实现 accounting period、双时区默认值与 Pending first reset
...
- 新部署默认 VPS refresh timezone `America/Los_Angeles`、client display timezone `Asia/Shanghai`；二者只接受有效 IANA timezone，不修改系统 timezone。
...
- [x] 自然月边界在 UTC 和非 UTC timezone 下计算正确。
- [x] 锚定日 1–31 均可配置，28/29/30/31 日在短月收敛到月末。
- [x] First reset instant 之前是合法 pending 状态，不返回错误。
- [x] DST 不存在和重复的本地时间均被拒绝。
- [x] 新部署默认 VPS refresh timezone 为 `America/Los_Angeles`、client display timezone 为 `Asia/Shanghai`；修改不会调用 `timedatectl` 或更改 VPS system timezone。
- [x] schema mismatch 能被明确识别并进入可诊断错误路径。
...
- 2026-09-04：根据 `CONTEXT.md` 的 Default timezone pair 决定，将新部署默认值改为 VPS refresh timezone `America/Los_Angeles` 与 client display timezone `Asia/Shanghai`；UTC 仍作为可显式选择的 accounting timezone。当前向导、配置校验和状态输出均按该决策实现。
...
- 2026-09-25：规格与当前 `CONTEXT.md`、`docs/implementation-plan.md`、`src/config.rs` 及配置向导测试对齐；库测试验证时区默认对、自然月边界、短月、DST、pending first reset 和 schema mismatch。

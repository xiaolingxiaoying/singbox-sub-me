# Runs INSIDE the Windows guest. Takes one screenshot per GUI page and writes a
# manifest that says *why* a page failed, so an empty table is a diagnosis
# rather than a guess.
param(
  [string]$Exe = 'C:\Users\Public\sbwin\sbgui.exe',
  [string]$OutDir = 'C:\Users\Public\sbwin\shots',
...
$manifest += "exe=" + $Exe
$manifest += "exe-exists=" + (Test-Path $Exe)
if (Test-Path $Exe) { $manifest += "exe-bytes=" + ((Get-Item $Exe).Length) }

# The UI ships Chinese strings but a guest installed from an English ISO may have
# no CJK face, in which case every glyph is a tofu box and a "passed" screenshot
# proves nothing. Report the font set before trusting any image.
...
$manifest += "guest-totalMB=" + [math]::Round($mem.TotalVisibleMemorySize / 1KB)
...
# strip of desktop into the shot. DWM's extended frame bounds do not.
...
function Invoke-Page([string]$page, [int]$attempt) {
  $env:SBGUI_PAGE = $page
  $env:SBGUI_SIZE = $Size
  if ($Lang) { $env:SBGUI_LANG = $Lang } else { Remove-Item env:SBGUI_LANG -ErrorAction SilentlyContinue }
  ...
  try {
    ...
    return "attempt$attempt LAUNCH-FAIL " + $_.Exception.Message
  }
  $targetPid = $p.Id
  ...
  Stop-Process -Id $targetPid -Force -ErrorAction SilentlyContinue
  return "OK ${w}x${ht} png=$((Get-Item $png).Length) title=[$($title.ToString())]"
}
...
foreach ($page in $Pages.Split(',')) {
  ...
  $manifest += "$page : $result freeMB=$free"
  Start-Sleep -Milliseconds 1500
}

$manifest += "finished=" + (Get-Date -Format o)
# A survivor holds the mixed port, which turns the NEXT page's auto-start into a
# "port already in use" failure that looks like a product bug.
$strays = (Get-Process sbgui, sbtui, sing-box -ErrorAction SilentlyContinue | ForEach-Object { $_.ProcessName }) -join ','
$manifest += "stray-processes=" + ($(if ($strays) { $strays } else { 'none' }))

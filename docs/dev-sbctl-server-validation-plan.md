...
3. Clash Party 导入订阅后的超时已定位到 VPS UFW：默认拒绝入站，仅放行 VLESS 当前端口；内核记录到 VMess 62892/TCP SYN 被丢弃。现已补齐 VMess、AnyTLS、Hysteria2、TUIC 当前端口的 UFW 规则，操作者工作站已确认所有 TCP 节点端口可连接。仍需用户在 Ubuntu VM 用 Clash Party 重测全部节点，确认 UDP 连通和端到端代理请求。

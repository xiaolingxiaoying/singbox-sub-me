...
3. Clash Party 导入订阅后的超时已定位到 VPS UFW：默认拒绝入站，仅放行 VLESS 当前端口；内核记录到 VMess 62892/TCP SYN 被丢弃。现已补齐 VMess、AnyTLS、Hysteria2、TUIC 当前端口的 UFW 规则；操作者工作站确认所有 TCP 节点端口可连接，用户确认 Ubuntu VM 的 Clash Party 五个节点均已恢复可用。独立 HTTPS 访问验证尚未收到用户确认。

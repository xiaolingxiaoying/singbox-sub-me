...
5. **限流跳过的谓词选错了。** 我用"模式 == external-proxy"，属性其实是"**对端是回环**"：
   ...
   → **本轮撤回这条建议**（理由见 R17 第 2 条：这条"属性"其实不如现状）。
...
2. 限流谓词从"模式"改成"对端是回环"，同时给 `verify.sh` 的洪水测试换一个非回环源。
   → **撤回（R16 第 5 条同此）**。把豁免挂在 `peer.is_loopback()` 上比现状更糟：本机反代拓扑下
   **所有人**都不再受限，而不是"共用一个桶"。`serve.rs:505-514` 的注释和 `subscription-guide.md`
   的安全边界一节已经把取舍写清楚了（转发头可伪造 → 限流会反变成存在性预言机；每客户端限额属于反代）。
   本轮只补了缺失的那句运维后果：`direct`/`ip-fallback` 被本机反代挡住时表现是**随机 429**，不是攻击。
...
## R23 — 对比度：把工单 02 第 6 条从形容词变成会红的门

数字来自 R22 之前那次量化（工单 02 末尾的表）。本轮落地：

- `MUTED` `#687774` → `#5d6b68`。旧值在纯白上 4.69 过关、在应用真正会画的四种底上
  4.09–4.43 全部不合格，所以"截图里看着没问题"从来不是证据。新值最差一对 4.86。
- 文字位置的 `FAINT`（18 处，2.5–2.8:1）→ `MUTED`；文字位置的 `CYAN`（8 处，3.3–3.7:1）
  → `CYAN_DARK`（最差 4.79）。`FAINT`/`CYAN` 保留为装饰 token，值不动。
- `AMBER` `#b45309` → `#9a4708`：旧值在导航底色上 4.38，正好压在 AA 线上，换个底就掉下去。
- `theme.rs` 末尾加 `contrast()`（`#[cfg(test)]`，运行时不需要它）与三道门：
  ① 5 个文字 token × 6 个底逐对断言 ≥4.5；② `Tone::Warning` 按钮的字/底是 `tone_colors` 里的
  字面量，不在 token 表里，单独钉一对；③ **扫自己的源码**，任何 `text_color(rgb(FAINT))` /
  `text_color(rgb(CYAN))` 出现就失败。

变异检验（两条新门都咬）：把 `MUTED` 改回旧值 → 门①红，且报的就是量化那五个数
（4.31 / 4.43 / 4.09 / 4.42 / 4.35，纯白 4.69 不在名单里，因为它确实过）；
在 `chrome.rs` 里塞一行含 `text_color(rgb(FAINT))` 的注释 → 门③红。

**门③的已知粗糙处**（写下来免得以后被当成精确工具）：它是**子串扫描**，不是 AST。
所以注释里提到这个写法也会红——我自己写测试说明时就先把自己绊倒了一次。
这是可接受的取舍：宁可误报一条注释，也不要漏掉一个真的低对比度标签；
报错信息带文件名，一眼能判。
...
## R27 续 — 帧到了，第一条就抓到 Bug

`shots-ticket01/860x640-subscriptions.png`（提交 `f528770` 之后抓到的那张）显示：

- 操作列在 860 下**折成两行**（第一对按钮一行，删除一行），行高被顶开；
- 更糟的是**最宽那一对会越出卡片右缘**：`860x640-subscriptions.png` 里未启用那一行是
  「设为当前 + 编辑链接」，这一对在 168 px 的轨道里放不下，`flex_wrap` 只在**轨道内**换行，
  而轨道本身比内容窄，于是折行后仍然顶到窗口边、最后一个字被裁。
  对照 `860x640-subscriptions-url-editor.png`：同一列在「更新 + 编辑链接」这对下是**正常**的
  （编辑器帧里看得很清楚，两个按钮都在卡片内），所以缺陷范围是"最宽标签对"，不是全部行。
  这正是工单 04 修过的那类缺陷（操作列被裁 → 不可点）被"加宽到 168 + `flex_wrap`"重新引入。

结论要写直白：**这批改动在窄屏是有可见缺陷的，不能说"GUI 没有 Bug"**。
功能本身（三条命令接线、纯函数、单测、N1–N7 变异）是立住的，坏的是那一格的宽度账。
修法已记进工单 01 新增第 8 项：窄屏改图标按钮（复用现成的 `mini_action`）或把操作列
...
## R30 — G11 信号处理与 G12 的 TUI 装载入口，以及我自己毁掉又恢复的一次工作

**G11（`fix(tui)` 4bf4764 + `test(dev)` 8327eb7）**：`run_app` 的循环本来就是
`tokio::select!`，所以修复只多一条分支——循环前订阅一次（每次迭代重装 handler 会漏掉迭代之间的信号），
循环里 `break`，于是被信号打断的退出与 `q` 走**同一个**出口，清理逻辑一行没复制。
`SIGINT` 故意不接：raw 模式下 Ctrl+C 是 crossterm 的按键，接了信号反而把"UI 卡死时唯一的出口"堵掉。
`should_clear_proxy(enabled, kept_on_purpose)` 抽成纯函数，两次变异（忽略用户选择 / 恒 false）都判红。

**这里最值钱的是 L2 门自己抓到我的假证**：第一版用 `pgrep -f "script -qeefc $BIN"` 找进程，
匹配到的是 **pty 包装器 `script`**，而 `script` 自己会接住 SIGTERM 并返回 0。
于是我跑出 `PASS SIGTERM: the teardown ran`——**测的根本不是 sbtui**。
揭穿它的正是我顺手加的控制组：同一目标发 `SIGKILL` 得到 137（不可捕获），
说明那个 0 来自包装器的退出码。现在改成按进程名匹配子进程、把 `wrapper=` 与 `sbtui=` 两个 pid 都打出来、
两者相同就拒绝跑这一例，并且轮询子进程消失（否则"handler 没生效"会表现为门挂住而不是门失败）。
教训写进长期记忆：**端到端的信号/退出码测试，控制组不是可选项。**

**G12 的 TUI 半边（`feat(tui)` 01cefff）**：`f` 在覆写页提示输入已存在文件的路径 → 读 → `SetOverride`。
四条"会丢用户工作"的边界都做了：覆盖已有覆写要第二次 `f`；确认条站着时别的键撤回；
装载用的是**第一次按下去时读到的那批字节**（不是二次确认时重读，文件可能已变）；
确认期间档案被切换就**拒绝**而不是装到"当前那个"上；空文件单独成句，因为空内容在引擎语义里是**清除**，
那是另一个键 `O` 的事。引擎侧没动生产代码——`SetOverride` 本来就校验并拒坏文档，
缺的是对 `apply` 分支本身的测试（此前所有测试都在直接调 `set_override`）。
变异 7 条：6 条判红；**P4（去掉空路径守卫）存活**，原因是那条测试从没按过空路径，
守卫没了 `read("")` 自己失败、"没发命令"照样成立——**这不是假绿测试，是覆盖边界**，
补一条"空路径要说'不能为空'而不是文件系统报错"的断言后 P4 判红。

**我自己造成的破坏与恢复**（写下来，因为这类错误只能靠流程记住）：
复跑 P4 之后我用 `git checkout -- crates/sbtui/src/input.rs` 想"只还原变异"，
但那次变异是在 **agent 未提交的工作**上做的，checkout 把它 398 行一起毁了。
靠事前存好的 `g12-input.diff` 用 `git apply --include=...` 恢复，再重加我自己的断言，
复跑 48/113 + clippy + fmt 全绿才提交。判据：**未提交工作在场时，禁止对受影响文件用裸
`git checkout`/`restore`/`stash`**；还原变异要么用内存里留着的内容写回，要么先 `git diff` 落盘。

**GUI 半边没做完**：writer agent 停在 150 轮上限，`crates/sbgui` 只剩纯函数与调用点、
页面 UI 一行没写（`pages/overrides.rs` 未改），`show_override_load` 是没人渲染的死状态，crate 编译不过。

...
| `s` | 启动 / 停止内核 | 全局 |
| `p` | 开 / 关系统代理 | 全局 |
| `m` | 切换 系统代理 / TUN 模式（下次启动生效；TUN 需管理员权限） | 全局 |
| `u` | 更新订阅 | 全局 |
| `↑↓` / `k j` | 移动选择 | 代理 / 连接 / 设置 |
...
- 内核从 sing-box 官方 Release 下载（zip 内含 wintun.dll），SHA-256 校验；
...
设置页修改 `auto_update_minutes`（直接编辑 `settings.toml`）后，内核运行期
会按该间隔自动拉取激活订阅；`0`（默认）关闭自动更新，`u` 手动更新始终可用。

...
sbtui 是 `client-core` 共享控制面之上的纯渲染层：内核下载、启停、自动重启、
订阅与设置全部由后台引擎承担，与桌面客户端 `sbgui` 走完全相同的引擎路径
（包括启动时按设置自动拉起内核）。
...
- 订阅支持 sbctl 四种格式链接 + 二维码/总览页链接（自动归一化），也支持纯
  sing-box JSON 链接与 Base64/明文 URI 列表（自动转换，并补齐入站、选择器
...
- 概览页的实时流量面板带有近 5 分钟的上下行速率 sparkline，并显示累计与峰值；
  「订阅与核心」面板显示内核版本、运行版本与内核内存占用。
- 「节点」页在手动测试前就显示内核最近上报的节点延迟，`T` 会并发探测整组延迟，
  失败节点显示为红色「超时」。
- 「连接」页展示目标、主机、网络、命中规则与出站链路，`/` 可按关键字过滤，
  `S` 切换排序。
- 「日志」页 `l` 按级别、`/` 按关键字过滤，`Space` 暂停，`c` 复制；
  按 `r` 跳到独立「入站」页（第 6 个页签），同屏展示内核正在使用的入站（类型 / 监听地址 / 端口）、
  规则集来源与自上而下的分流规则（引擎解析发布）；再按 `r` 返回日志页。

        ...
        match self.api.connections().await {
            Ok(snapshot) => {
                ...
                if let Some((last_up, last_down, last_time)) = self.last_totals {
                    ...
                    if elapsed > 0.05 {
                        self.up = ((totals.0 - last_up) as f64 / elapsed) as u64;
                        self.down = ((totals.1 - last_down) as f64 / elapsed) as u64;
                    }
                }
...
async fn run_app(
    ...
    // Leaving with the system proxy on would strand the user's traffic.
    if app.system_proxy_on {
        let _ = system_proxy::disable();
        app.log("已关闭系统代理（退出）。");
    }
    if app.core_child.is_some() {
        let _ = stop_core(&mut app).await;
    }
    Ok(())
}

async fn start_core(app: &mut App) -> Result<()> {
    if app.mode == TrafficMode::Tun && !system_proxy::can_use_tun() {
        anyhow::bail!(
...
async fn update_subscription(app: &mut App) -> Result<()> {
    ...
    let body = subscription::fetch(&url, &mirror).await?;
...
fn draw_logs(frame: &mut Frame, area: ratatui::prelude::Rect, app: &App) {
    let (title, lines) = if app.show_rules {
        (
            "分流规则（r 返回日志）",
            app.rules_lines
                .iter()
    ...
    } else if !app.core_logs.is_empty() {
        (
            "内核日志（r 查看规则）",
            app.core_logs
                .iter()
...
fn draw_settings(frame: &mut Frame, area: ratatui::prelude::Rect, app: &mut App) {
    ...
    let info = vec![
        ...
        Line::from(""),
        Line::from("快捷键: n 新增档案 ｜ u 更新订阅 ｜ v 改内核版本 ｜ r 改镜像 ｜ d 下载内核"),
    ];
    frame.render_widget(

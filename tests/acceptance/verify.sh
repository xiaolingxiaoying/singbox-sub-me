...
for path in sing-box.json clash.yaml uri; do
  ...
  contains "$response" 'subscription-userinfo: upload=30; download=60; total=999; expire='
done
...
mkdir -p "$root/etc/systemd/system" "$root/etc/nginx" "$root/etc/ufw"
